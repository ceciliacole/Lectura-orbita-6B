...
try{
 const payload={accessKey:APP_CONFIG.accessKey,codigo:$("#codigo").value.trim().toUpperCase(),grupo:"6ºB",titulo:$("#titulo").value.trim(),autor:$("#autor").value.trim(),valoracion:$('input[name="rating"]:checked').value,resumen:$("#resumen").value.trim(),pregunta:question,tipoRespuesta:mode,respuestaTexto:mode==="text"?$("#respuesta").value.trim():"",audioDataUrl:mode==="audio"?await dataUrl(audioBlob):""};
